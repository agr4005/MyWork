...
public class Ex01_Flow01 extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		// 1. Forward 방식
		// => flow01 -> hello
		// => 웹브라우저의 주소창은 바뀌지않으며, 요청명과 다른 Page가 출력됨.
		//	  요청명은 flow01이지만 실행결과는 hello
		
		// => 출력문이 출력되지 않음을 확인.
      	response.setContentType("text/html; charset = UTF-8");
		PrintWriter out = response.getWriter();
		out.print("<h2>**  Forward & Redirect Test  **</h2>");
		// => Console로 확인
...
//		request.getRequestDispatcher(uri).forward(request, response);
		...
		//  2. Redirect
		//	첫번째 요청 flow01에서 hellos라는 요청을 다시 보내줄것을 웹브라우저에게 response함.
		//  재요청으로 hellos가 서버로 전달되어 처리하는 방식
		//  => 그러므로 웹브라우저에게 표시된 요청명이 hellos로 변경됨.
		//  2.1) Servlet -> Servlet
		//  2.2) Servlet -> JSP
