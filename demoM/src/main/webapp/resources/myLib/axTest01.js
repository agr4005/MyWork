...
function rsLogin(){
	...
	fetch(url, {method :'Post',
       ...
       // => 1단계에서 return 한 Data 처리
		...
		if(err.message == '502') alert('id 또는 password 오류 입니다. 다시 시도해주세요.');
		else alert('시스템 오류입니다.');
	});
...
function rsLoginjj(){
	...
	fetch(url, {method :'Post',
		...
		alert(`response Data : ${responseData.id}, name =${responseData.username}`);
		location.reload();
	}).catch(err => {
		console.error(`** Error => ${err.message}`);
		if(err.message == '502') alert('id 또는 password 오류 입니다. 다시 시도해주세요.');
		else alert(`시스템 오류입니다. => ${err.message}`);
	});
...
function axiLoginjj() {
	...
	axios({url:url,
		...
		if(err.response.status == '502') alert('id 또는 password 오류 입니다. 다시 시도해주세요.');
		else alert(`시스템 오류입니다. => ${err.message}`);
	});
}
