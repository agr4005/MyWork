...
function idOK() {
	...
	// opener.document.getElementById('id').readonly='readonly';  // X
	// opener.document.getElementById('id').readOnly='readOnly';  // O
	opener.document.getElementById('id').readOnly = true;	  	  // O
	opener.document.getElementById('password').focus();
	//	4)
	close();
	//	window.close() 생략가능, self.close() 모두 가능
...
<div id="msgBlock">
<c:if test="${idUse=='T'}">
	${param.id} 는(은) 사용가능한 ID입니다. &nbsp;&nbsp;
	<button onclick="idOK()">ID 선택</button>
</c:if>
<c:if test="${idUse=='F'}">
	${param.id} 는(은) 사용중인 ID입니다. <br>
	다시 입력하세요. <br>
