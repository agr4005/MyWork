...
<tr bgcolor="DeepSkyBlue">
		...
		<c:if test="${!empty requestScope.banana}">
			...
			<c:if test="${b.indent>0}">
				...
				<span style="color:blue;"><b>re..</b></span>
			</c:if>
   ...
   => ver02 : makeQuery 메서드 -> searchQuery 메서드로 변경 -->
     ...
     <c:choose>
      ...
      <!-- ver01:  makeQuery 메서드 적용 
      		 <a href="bPageList${pageMaker.makeQuery(1)}">&#9664;</a>&nbsp;
      		<a href="bPageList${pageMaker.makeQuery(pageMaker.spageNo-1)}">&#8678;</a>&nbsp;&nbsp; 
      	   ver02 : makeQuery 메서드 -> searchQuery 메서드로 변경 -->
      		<a href="bPageList${pageMaker.searchQuery(1)}">&#9664;</a>&nbsp;
      		<a href="bPageList${pageMaker.searchQuery(pageMaker.spageNo-1)}">&#8678;</a>&nbsp;&nbsp;
      </c:when>
      <c:otherwise>
...
<!-- 3) Next, LastPage  -->
	<c:choose>
		<c:when test="${pageMaker.next && pageMaker.epageNo > 0}">
			&nbsp;<a href="bPageList${pageMaker.searchQuery(pageMaker.epageNo+1)}">&#8680;</a>
			&nbsp;<a href="bPageList${pageMaker.searchQuery(pageMaker.lastPageNo)}">&#9654;</a>
		</c:when>
		<c:otherwise>
			<font color="Gray">&nbsp;&#8680;&nbsp;&nbsp;&#9654;</font>
		</c:otherwise>
